<template>
  <div class="screen">
      ...
      <div class="scr-r">
        <span v-if="screen.name=='跟进时间'&&fixed" class="active">0-7天</span>
        <span v-else v-for="item in calcLength(screen.list)" :key="item.id" @click="choose(item,screen)" :class="{active:selectId && selectId==item.id}">{{item.name}}</span>
...
export default {
  ...
  methods:{
    ...
    deleteFilter(val){
      console.log(val)
      if(val == this.selectId || val == undefined){
        this.selectId = null
      }
   ...
   //销毁
  beforeDestroy(){
    $eventHub.$off("delete-filter",this.deleteFilter);
  } 
}
</script>
